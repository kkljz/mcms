...
<script type="text/javascript">
	function deleteRev(reserveId,userId,userType){
		if(confirm("是否确认取消预约？")){
		$.ajax({
...
<table class="tablelist">
    <thead>
        ...
        <th>创建时间</th>
        <c:if test="${loginuser.userType==1}"><th >预约类型</th></c:if>
        <th>实验室名称</th>
        <th>预约时间</th>
        <th>预约人数</th>
         <th>备注</th>
	...
	<tr>
	  ...
	  <td>${list.description }</td>
		<td>
			...
			<c:set var="reserveDate1">
				<fmt:formatDate value="${list.reserveDate }" pattern="yyyy-MM-dd" type="date"/>
			</c:set>

			<c:if test="${reserveDate1< currentday }">
				<input style="color: white" class="button btn-warning" type="button" value="不可取消预约" id="deletebtn" disabled="true"></input>
			</c:if>
			<c:if test="${reserveDate1>= currentday }">
				<%--<img src="${basePath }/static/images/t03.png"></img>--%>
				<input style="color: white"  class="button btn-success" type="button" value="取消预约" id="deletebtn"
					   onclick="deleteRev('${list.reserveId }','${loginuser.userId }','${loginuser.userType }');"></input>
			</c:if>
	  </td>
	</tr>
	</c:forEach>
