...
<c:if test="${loginuser.userType==0}"><input type="hidden" name="userId" id="userId" value='0' ></input></c:if>
<c:if test="${loginuser.userType!=0}"><input type="hidden" name="userId" id="userId" value="${loginuser.userId}" ></input></c:if>

<ul class="seachform">
    <li><label>用户类别：</label>
    <div class="vocation">
    <select name="userType" id="userType"  class="select1">
    ...
    <li><label>用户名：</label><input name="userName" id="userName" class="dfinput"  style="width:100px"></input></li>
	</c:if>
	<%--<c:if test="${loginuser.userType==0}">--%>
    <%--<li><label></label><input name="userName" id="userName" class="dfinput"  style="width:10px" type="hidden" value=""></input></li>--%>
	<%--</c:if>--%>
	
	<li><label>预约类型：</label>
    <div class="vocation">
    <select name="reserveType" id="reserveType"  class="select1">
...
<table class="tablelist">
    <thead>
        ...
        <th>用户名</th>
        <th>预约类型</th>
        <c:if test="${loginuser.userType!=2}"><th>课程名称</th></c:if>
        <th>预约时间</th>
        <th>预约人数</th>
        <th>备注</th>
        <th>创建时间</th>
	...
	<tr>
	  <td>${stauts.count}</td>
	  <td>${list.buildingName }${list.roomNumber }</td>
	  <td>${list.userName}</td>
	  <td>
	  		<c:if test="${list.reserveType==1 }">个人预约</c:if>
	  		<c:if test="${list.reserveType==2 }">课堂预约</c:if>
	  </td>
	  <c:if test="${loginuser.userType!=2}">
	  	<td>${list.courseName}</td>
	  </c:if>
	  
	  <td><fmt:formatDate value="${list.reserveDate }" pattern="yyyy-MM-dd"/>
	  	<c:if test="${list.timeInterval==1 }">  8:00~10:00</c:if>
